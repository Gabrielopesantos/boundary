...
type deleteTerminatedJob struct {
	...
	// the amount of time that a sessions must be in the terminated
...
func (d *deleteTerminatedJob) Run(ctx context.Context) error {
	...
	d.deletedInRun, err = d.repo.deleteTerminated(ctx, d.threshold)
	if err != nil {
...
func (d *deleteTerminatedJob) Description() string {
	return "Delete terminated sessions"
}
